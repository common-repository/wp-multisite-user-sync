...
if ( ! function_exists( 'wmus_user_sync_unsync' ) ) {
    ...
    function wmus_user_sync_unsync() {
        ...
        ?>
            <h2><?php esc_html_e( 'WordPress Multisite User Sync/Unsync' ); ?></h2>
            <table class="form-table">
                <tbody>
                    <tr>
                        <th><label><?php esc_html_e( 'Sync/Unsync?' ); ?></label></th>
                        <td>
                            <fieldset>
                                <label>
                                    <input type="radio" name="wmus_sync_unsync" value="<?php echo esc_attr( '1' ); ?>" checked="checked" /><?php esc_attr_e( 'Sync' ); ?>
                                </label>
                                <label>
                                    <input type="radio" name="wmus_sync_unsync" value="<?php echo esc_attr( '0' ); ?>" /><?php esc_attr_e( 'Unsync' ); ?>
                                </label>
                            </fieldset>
                            <p class="description"><?php esc_html_e( 'Select sync/unsync.' ); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><label><?php esc_html_e( 'Sites' ); ?></label></th>
                        <td>
                            <label><input class="wmus-check-uncheck" type="checkbox" /><?php esc_html_e( 'All' ); ?></label>
                            <p class="description"><?php esc_html_e( 'Select/Deselect all sites.' ); ?></p>
                            <br>
                                    ...
                                    if ( $sites != null ) {
                                        ...
                                        foreach ( $sites as $key => $value ) {
                                            ...
                                            if ( ( $value->blog_id != get_current_blog_id() ) || ( is_network_admin() ) ) {
                                                ?>
                                                    <label><input name="wmus_blogs[]" type="checkbox" value="<?php echo esc_attr( $value->blog_id ); ?>"<?php echo esc_attr( $checked ); ?>><?php echo esc_html( $value->domain ); echo esc_html( $value->path ); echo ' ('.esc_html( $blog_details->blogname ).')'; ?></label><br>
                                                <?php
                                            }
                                        }
                                    }
                            ...
                            <p class="description"><?php esc_html_e( 'Select destination sites you want to sync/unsync.' ); ?></p>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        ?>
            <div class="wrap">
                ...
                <table class="form-table">
                    <tbody> 
                        <tr>
                            <th scope="row"><?php esc_html_e( 'User Roles' ); ?></th>
                            <td>
                                <fieldset>
                                    <?php
                                        $checked = '';
                                    ...
                                    ?>
                                    <label><input name="wmus_user_roles" type="checkbox" value="0"<?php echo esc_attr( $checked ); ?>><?php esc_attr_e( 'None' ); ?></label><br>
                                    <?php
                                        $roles = get_editable_roles();
                                        if ( $roles != null ) {
                                            foreach ( $roles as $key => $value ) {
                                                if ( $key != 'subscriber' ) {
                                                    ...
                                                    ?>
                                                        <label><input name="wmus_user_roles[]" type="checkbox" value="<?php echo esc_attr( $key ); ?>"<?php echo esc_attr( $checked ); ?>><?php echo esc_html( $value['name'] ); ?></label><br>
                                                    <?php
                                                }
                                            }
                                        }
                ...
                <p class="submit"><input name="wmus_submit" class="button button-primary" value="<?php esc_attr_e( 'Save Changes' ); ?>" type="submit"></p>
                </form>
            </div>
        <?php
    }
